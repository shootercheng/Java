...
//		 logger.info("--------one day start time date------------"+date);
    ...
    public static void main(String[] args) throws ParseException {
        ...
        // 日志时间周期
        int intervalsec = FIVE_MIN_INTERVAL;
        Long endTime = getPreGranularityTime(System.currentTimeMillis(),intervalsec);
        long startTime = getStartTime(endTime, THIRTY_DAY_INTERVAL, intervalsec);
        System.out.println(formatMillisecondstoString(endTime, YYYY_MM_DD_HH_MM_SS));
        List<Long> timeslist = getTimes(endTime, intervalsec, MULTIPLYING_RATE);
        ...
        System.out.println("排序后的点================================");
        sortMap(pointList);
        for(Map.Entry<Long, BigInteger> entry : pointList){
            System.out.println("time:" + DateUtil.formatMillisecondstoString(entry.getKey(),YYYY_MM_DD_HH_MM_SS) + ",filesize:" + entry.getValue());
        }
    }
}
